...
namespace Microsoft.IdentityModel.Tokens
{
    ...
    {        
        ...
        //public override SecurityKeyIdentifierClause CreateSecurityTokenReference(System.IdentityModel.Tokens.SecurityToken token, bool attached)
        //{
        ...
        protected virtual ClaimsIdentity CreateClaimsIdentity(Saml2SecurityToken samlToken, string issuer, TokenValidationParameters validationParameters)
        {
            ...
            // TODO we need to translate to different token?
            SecurityToken token = null;
            ClaimsIdentity identity = validationParameters.CreateClaimsIdentity(token, issuer);
        ...
        public override SecurityToken ReadToken(XmlReader reader, TokenValidationParameters validationParameters)
        {
            ...
            var samlSecurityToken = new Saml2Handler()
            {
            ...
            try
            {
                    ...
                    using (XmlDictionaryReader reader = XmlDictionaryReader.CreateDictionaryReader(XmlReader.Create(sr)))
                    {
                        samlToken = (new Saml2Handler()
                        {
                            Configuration = new SecurityTokenHandlerConfiguration
                            {
                                ...
                                MaxClockSkew = validationParameters.ClockSkew
                                //ServiceTokenResolver = SecurityTokenResolver.CreateDefaultSecurityTokenResolver(validationParameters.TokenDecryptionKeys, true),
                            }
                ...
                if (validationParameters.ValidateLifetime)
                {
                    if (validationParameters.LifetimeValidator != null)
                    {
                        ...
                        if (!validationParameters.LifetimeValidator(notBefore: notBefore, expires: expires, securityToken: null, validationParameters: validationParameters))
                        {
                    ...
                    {
                        ValidateLifetime(notBefore: notBefore, expires: expires, securityToken: null, validationParameters: validationParameters);
                    }
                }
                ...
                if (validationParameters.ValidateAudience)
                {
                    ...
                    {
                        if (!validationParameters.AudienceValidator(audiences, null, validationParameters))
                        {
                    ...
                    {
                        ValidateAudience(audiences, null, validationParameters);
                    }
                }
                ...
                if (validationParameters.ValidateIssuer)
                {
                    ...
                    {
                        issuer = validationParameters.IssuerValidator(issuer, null, validationParameters);
                    }
                    else
                    {
                        issuer = ValidateIssuer(issuer, null, validationParameters);
                    }
                }

                if (samlToken.IssuerToken != null)
                {
                    ValidateIssuerSecurityKey(null, null, validationParameters);
